...
files=$( ls *.cpp *.c *.h Makefile.* web_pre.js )

src_root=$(pwd)

build="klejne_web_build"
...
if [ "$2" = "" ] ; then
	...
	echo "Copies source files to <newroot>/$build"
...
chroot $newroot /bin/bash -c "PATH=\"/$emsdk/$toolchain:\$PATH\" make -C /$build -f Makefile.web"

expand_artifacts() {
	echo $1klejne{.js,.wasm,.aw.js,.ww.js}
}
...
cp $(expand_artifacts $newroot/$build/) $src_root
chown $(stat -f"%Su:%Sg" $src_root) $(expand_artifacts) # XXX stat usage is FreeBSD specific
